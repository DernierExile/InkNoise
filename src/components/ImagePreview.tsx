...
import { Download, Columns2, SlidersHorizontal, ChevronDown, X } from 'lucide-react';
...
export default function ImagePreview({ originalImage, processedImageData, onExport }: ImagePreviewProps) {
  ...
  // Draw the processed image at native (100%) resolution into the zoom
  // overlay canvas when it opens. Lets the user inspect grain and detail
  // pixel-for-pixel, which the fit-to-panel preview can hide.
  useEffect(() => {
    if (zoomOpen && processedImageData) {
      drawToCanvas(zoomRef.current, processedImageData);
    }
  }, [zoomOpen, processedImageData]);
  ...
  return (
    <div className="space-y-2">
      ...
      {zoomOpen && processedImageData && (
        <div
          className="fixed inset-0 z-[120] bg-bz-graphite/95 backdrop-blur-sm overflow-auto cursor-zoom-out flex items-start justify-center p-6"
          onClick={() => setZoomOpen(false)}
          role="dialog"
          aria-modal="true"
        >
          <div className="absolute top-3 left-4 text-[10px] font-mono-ui text-bz-cyan tracking-[0.2em] uppercase pointer-events-none">
            {processedImageData.width} × {processedImageData.height} · 100%
          </div>
          <button
            onClick={(e) => { e.stopPropagation(); setZoomOpen(false); }}
            className="fixed top-3 right-4 z-[121] w-7 h-7 flex items-center justify-center bg-bz-deep border border-bz-grid text-bz-system hover:text-bz-paper hover:border-bz-system transition-colors duration-240"
