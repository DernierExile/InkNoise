...
export default function ControlPanel({
  ...
  const t = useT();
  const basePalette = PREDEFINED_PALETTES[selectedPalette]?.colors ?? [];
  const previewPalette = applyPaletteModifiers(basePalette, paletteModifiers);
  ...
  return (
    <div className="panel p-3 max-h-[calc(100vh-7rem)] overflow-y-auto control-panel-scroll">
      <div className="pb-2.5 mb-1 border-b border-bz-grid">
        ...
        <button
          onClick={onResetAll}
          className="mt-2 w-full flex items-center justify-center gap-1.5 px-2 py-1.5 text-[10px] font-mono-ui text-bz-system border border-bz-grid hover:border-bz-cyan hover:text-bz-cyan transition-colors duration-240 tracking-widest uppercase"
          title={t('controlPanel.button.resetAll')}
        >
          <RotateCcw className="w-2.5 h-2.5" />
          {t('controlPanel.button.resetAll')}
        </button>
      </div>
      ...
      <Section title={t('controlPanel.section.palette')} defaultOpen={true}>
        ...
        <div className="flex gap-0.5 flex-wrap">
          {previewPalette.map((color, idx) => (
            <div key={idx} className="w-4 h-4 border border-bz-grid flex-shrink-0 transition-transform duration-240 hover:scale-150 cursor-default"
              style={{ backgroundColor: color }} title={basePalette[idx] === color ? color : `${basePalette[idx]} -> ${color}`} />
          ))}
        </div>
        ...
        <Slider label={t('controlPanel.slider.paletteSaturation')} value={paletteModifiers.saturationBoost} min={-100} max={100}
        ...
        <button onClick={() => onPaletteModifiersChange({ hueShift: 0, saturationBoost: 0, brightnessShift: 0, intensity: 100 })}
          className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 text-[10px] font-mono-ui text-bz-system border border-bz-grid hover:border-bz-cyan hover:text-bz-cyan transition-colors duration-240 tracking-widest uppercase"
        >
          <RotateCcw className="w-2.5 h-2.5" />
          {t('controlPanel.button.resetPalette')}
        </button>
      </Section>

      <Section title={t('controlPanel.section.postProcessing')} defaultOpen={true}>
        ...
        <Slider label={t('controlPanel.slider.vignette')} value={postProcessing.vignette} min={0} max={100} step={0.1}
          onChange={(v) => onPostProcessingChange({ ...postProcessing, vignette: v })} display={formatPercent(postProcessing.vignette)} />
      ...
      <Section title={t('controlPanel.section.effects')} defaultOpen={true}>
      ...
      <Section title={t('controlPanel.section.tonal')} defaultOpen={true}>
      ...
      <Section title={t('controlPanel.section.levels')} defaultOpen={true}>
      ...
      <Section title={t('controlPanel.section.adjustments')} defaultOpen={true}>
        ...
        <Slider label={t('controlPanel.slider.imageSaturation')} value={adjustments.saturation} min={0} max={200}
        ...
        <div className="grid grid-cols-1 gap-1.5">
          <button onClick={() => onAdjustmentsChange({
            ...
            className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 text-[10px] font-mono-ui text-bz-system border border-bz-grid hover:border-bz-system hover:text-bz-interface transition-colors duration-240 tracking-widest uppercase"
          >
            <RotateCcw className="w-2.5 h-2.5" />
            {t('controlPanel.button.resetAdjustments')}
          </button>
          <button onClick={onReAnalyze}
            className="w-full flex items-center justify-center gap-1.5 px-2 py-1.5 text-[10px] font-mono-ui text-bz-cyan border border-bz-grid hover:border-bz-cyan transition-colors duration-240 tracking-widest uppercase"
          >
      ...
      <Section title={t('controlPanel.section.inputOutput')} defaultOpen={true}>
        <div>
      ...
      <div className="pt-3 mt-1 border-t border-bz-grid">
        <button
          onClick={onResetAll}
          className="w-full flex items-center justify-center gap-1.5 px-2 py-2 text-[10px] font-mono-ui text-bz-system border border-bz-grid hover:border-bz-cyan hover:text-bz-cyan transition-colors duration-240 tracking-widest uppercase"
          title={t('controlPanel.button.resetAll')}
        >
