...
const PRODUCTION_PACK_FILES = [
  { key: 'pack', href: '/samples/inknoise-production-pack.zip' },
  { key: 'recipe', href: '/samples/production-pack/inknoise-production-recipe.json' },
  { key: 'output1', href: '/samples/production-pack/output/campaign-01-editorial-inknoise.png' },
  { key: 'output2', href: '/samples/production-pack/output/campaign-02-food-inknoise.png' },
] as const;
...
const QUICK_LOOKS: QuickLook[] = [
  {
    id: 'editorial',
    title: 'Editorial grain',
    body: 'Press-kit images, newsprint texture, sharp black fields.',
    src: '/samples/09-editorial.jpg',
    algo: 'halftone-circle',
    opts: { cell: 5, angle: 28, threshold: 128, fg: [244, 244, 241], bg: [5, 6, 7] },
    tags: ['Magazine', 'Newsprint', 'Mono'],
  },
  {
    id: 'riso-food',
    title: 'Riso food poster',
    body: 'High contrast campaign texture with a warm spot-color feel.',
    src: '/samples/05-food.jpg',
    algo: 'floyd-steinberg',
    opts: { threshold: 120, fg: [232, 74, 31], bg: [5, 6, 7] },
    tags: ['Poster', 'Riso', 'Food'],
  },
  {
    id: 'portrait',
    title: 'Portrait halftone',
    body: 'Skin, hair and fabric pushed into graphic dot structure.',
    src: '/samples/02-portrait.jpg',
    algo: 'atkinson',
    opts: { threshold: 132, fg: [244, 244, 241], bg: [5, 6, 7] },
    tags: ['Portrait', 'Atkinson', 'Grain'],
  },
  {
    id: 'object',
    title: 'Product texture',
    body: 'Object shots with hard edges, shadow control and clean contrast.',
    src: '/samples/01-object.jpg',
    algo: 'bayer-8',
    opts: { threshold: 118, fg: [0, 213, 255], bg: [5, 6, 7] },
    tags: ['Product', 'Bayer', 'Cyan'],
  },
  {
    id: 'anime',
    title: 'Screentone shade',
    body: 'Flat-color images converted into controlled comic-style texture.',
    src: '/samples/08-anime.jpg',
    algo: 'halftone-line',
    opts: { cell: 4, angle: 42, threshold: 126, fg: [244, 211, 94], bg: [5, 6, 7] },
    tags: ['Comic', 'Line', 'Shade'],
  },
  {
    id: 'city',
    title: 'City signal',
    body: 'Urban scenes with a tighter signal-processing and poster feel.',
    src: '/samples/10-cityscape.jpg',
    algo: 'stucki',
    opts: { threshold: 124, fg: [83, 97, 255], bg: [244, 244, 241] },
    tags: ['Urban', 'Signal', 'Poster'],
  },
];

export function QuickLooks({ onTrySample }: { onTrySample: (src: string) => void }) {
  return (
    <section className="border-t border-bz-grid py-16 sm:py-20 px-4 sm:px-6">
      <div className="max-w-[1400px] mx-auto">
        <div className="grid md:grid-cols-[320px_1fr] gap-10 md:gap-12 mb-10 items-end">
          <div>
            <div className="flex items-baseline gap-2 mb-3">
              <span className="font-mono-ui text-[10px] tracking-[0.22em] uppercase text-bz-system">01</span>
              <span className="font-mono-ui text-[10px] tracking-[0.22em] uppercase text-bz-system">Pick a look</span>
            </div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-medium tracking-[-0.02em] text-bz-paper leading-[1.1]">
              Start from an outcome, not a manual.
            </h2>
          </div>
          <p className="text-bz-interface leading-relaxed max-w-[620px]">
            The algorithm catalog can wait. First, choose the kind of image you want to make, open it in the workbench, and tune from there.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-2">
          {QUICK_LOOKS.map((look) => (
            <QuickLookCard key={look.id} look={look} onTrySample={onTrySample} />
          ))}
        </div>
      </div>
    </section>
...
function QuickLookCard({ look, onTrySample }: { look: QuickLook; onTrySample: (src: string) => void }) {
  const source = useSourceImage(look.src, 560);

  return (
    <article className="panel-surface overflow-hidden flex flex-col min-h-[420px]">
      <div className="relative aspect-[4/3] bg-bz-graphite border-b border-bz-grid overflow-hidden">
        {source ? <DitherCanvas source={source} algo={look.algo} opts={look.opts} /> : null}
        <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
          {look.tags.map((tag) => (
            <span key={tag} className="font-mono-ui text-[9px] tracking-[0.18em] uppercase text-bz-paper bg-bz-graphite/80 border border-bz-grid px-2 py-1">
              {tag}
            </span>
          ))}
        </div>
      ...
      <div className="p-5 flex flex-1 flex-col gap-3">
        <div className="flex-1">
          <h3 className="text-xl text-bz-paper font-medium tracking-tight">{look.title}</h3>
          <p className="mt-2 text-[13px] leading-relaxed text-bz-interface">{look.body}</p>
        </div>
        <button
          type="button"
          onClick={() => onTrySample(look.src)}
          className="inline-flex w-full items-center justify-center gap-2 px-4 py-3 border border-bz-grid hover:border-bz-cyan transition-colors duration-240 font-mono-ui text-[10px] tracking-[0.18em] uppercase text-bz-paper"
        >
          Open this image
        </button>
      </div>
    ...
    <section className="border-t border-bz-grid py-20 px-4 sm:px-6">
      <div className="max-w-[1400px] mx-auto">
        <div className="max-w-[720px] mx-auto text-center mb-12">
          ...
          <h2 className="mt-3 text-3xl sm:text-4xl md:text-5xl font-medium tracking-[-0.02em] text-bz-paper leading-[1.1]">
            Not a filter. <span className="text-bz-system">Not a wrapper.</span> An engine.
          </h2>
          <p className="mt-4 text-bz-interface leading-relaxed">
            InkNoise sits between your source files and your output medium · Riso press, plotter, halftone newsprint, sub-pixel display. Deterministic, parametric, reproducible. The opposite of "magic mode": same input, same output, every time, on every machine.
          </p>
        </div>
...
export function Workflow() {
  const steps = [
    { n: 'Step 01', h: 'Drop. Preview live.', p: 'Drag a single image, or a folder of fifty. The preview canvas updates at 16ms per render · no spinner, no upload, no server.', glyph: <DropGlyph />, bg: 'radial-gradient(120% 120% at 72% 24%, rgba(0,213,255,0.16), transparent 58%), linear-gradient(160deg,#191920,#0a0a0d)' },
    { n: 'Step 02', h: 'Save preset.',        p: 'Every parameter · algorithm, palette, post-stack, threshold curve · serializes to a portable JSON file. Drag it back in to restore the exact look on any machine.', glyph: <PresetGlyph />, bg: 'radial-gradient(120% 120% at 28% 76%, rgba(232,74,31,0.16), transparent 58%), linear-gradient(160deg,#191920,#0a0a0d)' },
    { n: 'Step 03', h: 'Batch the campaign.', p: 'Drop the same preset on a folder of 50 images. Get a zip back with consistent texture across every visual. Same input, same output. No drift.', glyph: <BatchGlyph />, bg: 'radial-gradient(120% 120% at 50% 38%, rgba(0,213,255,0.13), transparent 60%), linear-gradient(160deg,#191920,#0a0a0d)' },
  ];
    ...
    <section className="border-t border-bz-grid py-20 px-4 sm:px-6">
      <div className="max-w-[1400px] mx-auto">
        <div className="max-w-[680px] mx-auto text-center mb-12">
          ...
          <h2 className="mt-3 text-3xl sm:text-4xl md:text-5xl font-medium tracking-[-0.02em] text-bz-paper leading-[1.1]">
            One image, fifty images, the same look.
          </h2>
          <p className="mt-4 text-bz-interface leading-relaxed">
            InkNoise is built for a campaign, not a single hero shot. Drop, pick the version that works, then run the exact same recipe across every visual you ship.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {steps.map((s) => (
            <div key={s.n} className="group rounded-xl overflow-hidden border border-bz-grid bg-bz-deep transition-transform duration-240 hover:-translate-y-1">
              <div className="relative h-48 overflow-hidden border-b border-bz-grid" style={{ background: s.bg }}>
                {s.glyph}
              </div>
              <div className="p-6 flex flex-col gap-2">
                <span className="font-mono-ui text-[10px] tracking-[0.22em] uppercase text-bz-cyan">{s.n}</span>
                <h4 className="text-xl text-bz-paper font-medium tracking-tight">{s.h}</h4>
                <p className="text-[13px] text-bz-interface leading-relaxed">{s.p}</p>
              </div>
            </div>
          ))}
        </div>
...
// ─── Showcase · 3 exemples tirés des assets (même grammaire que Workflow) ─────

interface ShowcaseEx { img: string; tag: string; title: string; body: string; }

const SHOWCASE_EX: ShowcaseEx[] = [
  { img: '/showcase/1.jpg', tag: 'Halftone', title: 'Editorial halftone', body: 'Fine halftone grid, deep blacks · press and poster ready.' },
  { img: '/showcase/2.jpg', tag: 'Duotone', title: 'Neon duotone', body: 'Two-ink street energy · warm highlights, electric shadows.' },
  { img: '/showcase/3.jpg', tag: 'Riso', title: 'Riso pop', body: 'CMYK-style dots in punchy spot colors · zine and poster look.' },
];

function ShowcaseCard({ ex }: { ex: ShowcaseEx }) {
  return (
    <div className="group rounded-xl overflow-hidden border border-bz-grid bg-bz-deep transition-transform duration-240 hover:-translate-y-1">
      <div className="relative aspect-[4/3] overflow-hidden border-b border-bz-grid bg-bz-graphite">
        <img src={ex.img} alt={ex.title} loading="lazy" className="absolute inset-0 w-full h-full object-cover" />
        <span className="absolute top-3 left-3 font-mono-ui text-[9px] tracking-[0.18em] uppercase text-bz-paper bg-bz-graphite/80 border border-bz-grid px-2 py-1">{ex.tag}</span>
      </div>
      <div className="p-6 flex flex-col gap-2">
        <h4 className="text-xl text-bz-paper font-medium tracking-tight">{ex.title}</h4>
        <p className="text-[13px] text-bz-interface leading-relaxed">{ex.body}</p>
      </div>
    </div>
  );
}
...
export function Showcase() {
  return (
    <section className="border-t border-bz-grid py-20 px-4 sm:px-6">
      <div className="max-w-[1400px] mx-auto">
        <div className="max-w-[680px] mx-auto text-center mb-12">
          <span className="font-mono-ui text-[10px] tracking-[0.22em] uppercase text-bz-system">Examples</span>
          <h2 className="mt-3 text-3xl sm:text-4xl md:text-5xl font-medium tracking-[-0.02em] text-bz-paper leading-[1.1]">
            One engine, every kind of image.
          </h2>
          <p className="mt-4 text-bz-interface leading-relaxed">
            Photos, campaign assets, AI renders, product shots · drop anything and pick a direction. Three, straight from real shots.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {SHOWCASE_EX.map((ex) => <ShowcaseCard key={ex.src} ex={ex} />)}
        </div>
      </div>
    </section>
  );
...
export function ProductionPack() {
  const t = useT();
          ...
          <div>
            <div className="flex items-baseline gap-2 mb-3">
              ...
              <span className="font-mono-ui text-[10px] tracking-[0.22em] uppercase text-bz-system">
                {t('home.marketing.productionPack.label')}
              </span>
            </div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-medium tracking-[-0.02em] text-bz-paper leading-[1.1]">
              {t('home.marketing.productionPack.title')}
            </h2>
          </div>
          <p className="text-bz-interface leading-relaxed max-w-[560px]">
            {t('home.marketing.productionPack.body')}
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-[1.15fr_1.35fr] gap-4">
          <div className="panel-surface overflow-hidden">
            <div className="grid grid-cols-2 gap-px bg-bz-grid">
              <img src="/samples/production-pack/output/campaign-01-editorial-inknoise.png" alt="" className="aspect-[4/3] w-full object-cover bg-bz-deep" loading="lazy" />
              <img src="/samples/production-pack/output/campaign-02-food-inknoise.png" alt="" className="aspect-[4/3] w-full object-cover bg-bz-deep" loading="lazy" />
            </div>
            <div className="p-5">
              <p className="font-mono-ui text-[10px] tracking-[0.18em] uppercase text-bz-system mb-2">
                {t('home.marketing.productionPack.previewLabel')}
              </p>
              <p className="text-[13px] leading-relaxed text-bz-interface">
                {t('home.marketing.productionPack.previewBody')}
              </p>
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {PRODUCTION_PACK_FILES.map(file => (
              <a
                key={file.key}
                href={file.href}
                download
                className="panel-surface p-5 flex flex-col gap-3 hover:border-bz-cyan transition-colors duration-240"
              >
                <span className="font-mono-ui text-[10px] tracking-[0.18em] uppercase text-bz-cyan">
                  {t(`home.marketing.productionPack.files.${file.key}.meta`)}
                </span>
                <span className="text-bz-paper font-medium">
                  {t(`home.marketing.productionPack.files.${file.key}.name`)}
                </span>
                <span className="text-[12px] leading-relaxed text-bz-interface">
                  {t(`home.marketing.productionPack.files.${file.key}.desc`)}
                </span>
              </a>
            ))}
          </div>
...
function DropGlyph() {
  return (
    <svg viewBox="0 0 320 150" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
      <defs>
        <linearGradient id="dg-photo" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0" stopColor="#36507e" />
          <stop offset="0.55" stopColor="#7a5a8a" />
          <stop offset="1" stopColor="#e8915a" />
        </linearGradient>
        <pattern id="dg-dots" width="9" height="9" patternUnits="userSpaceOnUse">
          <circle cx="2.2" cy="2.2" r="1.7" fill="#f4f4f1" />
        </pattern>
        <clipPath id="dg-clip"><rect x="86" y="34" width="148" height="100" rx="8" /></clipPath>
      </defs>
      <rect x="86" y="34" width="148" height="100" rx="8" fill="#0a0a0c" stroke="#2b2b34" />
      <g clipPath="url(#dg-clip)">
        <rect x="86" y="34" width="74" height="100" fill="url(#dg-photo)" />
        <rect x="160" y="34" width="74" height="100" fill="#0a0a0c" />
        <rect x="160" y="34" width="74" height="100" fill="url(#dg-dots)" opacity="0.92" />
      </g>
      <line x1="160" y1="34" x2="160" y2="134" stroke="#00d5ff" strokeWidth="1.5" />
      <circle cx="160" cy="18" r="11" fill="#00d5ff" fillOpacity="0.16" />
      <path d="M160 8 L160 25 M152 18 L160 26 L168 18" stroke="#00d5ff" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
function PresetGlyph() {
  return (
    <svg viewBox="0 0 320 150" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
      <rect x="66" y="16" width="188" height="120" rx="10" fill="#101016" stroke="#2b2b34" />
      <path d="M66 26 a10 10 0 0 1 10 -10 h168 a10 10 0 0 1 10 10 v8 h-188 z" fill="#16161c" />
      <circle cx="80" cy="26" r="3" fill="#E84A1F" />
      <circle cx="92" cy="26" r="3" fill="#f4d35e" />
      <circle cx="104" cy="26" r="3" fill="#1FA56D" />
      <text x="244" y="29" fontSize="9" fill="#6a6a72" textAnchor="end" fontFamily="monospace">preset.json</text>
      {[0, 1, 2, 3].map((i) => (
        <g key={i} transform={`translate(82 ${52 + i * 20})`}>
          <rect x="0" y="0" width="46" height="6" rx="3" fill="#33333c" />
          <rect x="58" y="0" width="98" height="6" rx="3" fill="#22222a" />
          <rect x="58" y="0" width={[72, 40, 88, 54][i]} height="6" rx="3" fill={i === 1 ? '#E84A1F' : '#00d5ff'} fillOpacity={i === 1 ? 0.9 : 0.8} />
        </g>
      ))}
      <circle cx="226" cy="116" r="13" fill="#00d5ff" fillOpacity="0.16" />
      <path d="M220 116 l4 4 l8 -9" stroke="#00d5ff" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
function BatchGlyph() {
  const tex = ['#00d5ff', '#E84A1F', '#f4d35e', '#1FA56D', '#ff3d7f', '#5361ff', '#00d5ff', '#E84A1F', '#1FA56D'];
  return (
    <svg viewBox="0 0 320 150" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
      <defs>
        <pattern id="bg-dots" width="6" height="6" patternUnits="userSpaceOnUse"><circle cx="1.5" cy="1.5" r="1.1" fill="#f4f4f1" fillOpacity="0.5" /></pattern>
      </defs>
      {Array.from({ length: 9 }).map((_, i) => {
        const col = i % 3, row = Math.floor(i / 3);
        const x = 60 + col * 50, y = 14 + row * 42;
        const hl = i === 4;
        return (
          <g key={i}>
            <rect x={x} y={y} width="42" height="34" rx="5" fill="#0c0c10" stroke={hl ? '#00d5ff' : '#2b2b34'} strokeWidth={hl ? 1.5 : 1} />
            <rect x={x} y={y} width="42" height="34" rx="5" fill="url(#bg-dots)" opacity={hl ? 0.85 : 0.3} />
            <rect x={x + 6} y={y + 23} width="18" height="3" rx="1.5" fill={tex[i]} fillOpacity={hl ? 0.95 : 0.5} />
          </g>
        );
      })}
      <path d="M222 75 h26 M241 68 l8 7 l-8 7" stroke="#00d5ff" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round" />
      <rect x="252" y="62" width="44" height="26" rx="6" fill="#101016" stroke="#2b2b34" />
      <text x="274" y="78" fontSize="10" fill="#00d5ff" textAnchor="middle" fontFamily="monospace">.zip</text>
    </svg>
  );
}
