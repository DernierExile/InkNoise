...
export function Manifesto() {
  const t = useT();
  const p = 'home.marketing.manifesto';
    ...
    <section className="border-t border-bz-grid py-20 px-4 sm:px-6">
      <div className="max-w-[1400px] mx-auto">
        <div className="grid md:grid-cols-[320px_1fr] gap-12 mb-12 items-baseline">
          <div>
            <div className="flex items-baseline gap-2 mb-3">
              ...
              <span className="font-mono-ui text-[10px] tracking-[0.22em] uppercase text-bz-system">{t(`${p}.eyebrow`)}</span>
            </div>
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-medium tracking-[-0.02em] text-bz-paper leading-[1.1]">
              {t(`${p}.title.part1`)} <span className="text-bz-system">{t(`${p}.title.part2Muted`)}</span> {t(`${p}.title.part3`)}
            </h2>
          </div>
          <p className="text-bz-interface leading-relaxed max-w-[560px]">
            {t(`${p}.intro`)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 mb-8">
          <ManifestoCell variant="neg" title={t(`${p}.cells.filter.title`)}
            body={t(`${p}.cells.filter.body`)} />
          <ManifestoCell variant="neg" title={t(`${p}.cells.ai.title`)}
            body={t(`${p}.cells.ai.body`)} />
          <ManifestoCell variant="neg" title={t(`${p}.cells.photoshop.title`)}
            body={t(`${p}.cells.photoshop.body`)} />
          <ManifestoCell variant="pos" title={t(`${p}.cells.inknoise.title`)}
            body={t(`${p}.cells.inknoise.body`)} />
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {[
            { k: t(`${p}.stats.algorithms`), v: '25' },
            { k: t(`${p}.stats.colorModels`), v: '8' },
            { k: t(`${p}.stats.postStacks`), v: '6' },
            { k: t(`${p}.stats.export`), v: '4K · PNG · TIFF · SVG' },
          ].map((cell) => (
...
export function Workflow() {
  const t = useT();
  const p = 'home.marketing.workflow';
  const steps = [
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          {steps.map((s) => (
            <div key={s.key} className="panel-surface p-6 flex flex-col gap-3 min-h-[280px]">
              <span className="font-mono-ui text-[10px] tracking-[0.22em] uppercase text-bz-system">{t(`${p}.steps.${s.key}.n`)}</span>
              <h4 className="text-xl text-bz-paper font-medium tracking-tight">{t(`${p}.steps.${s.key}.h`)}</h4>
              <p className="text-[13px] text-bz-interface leading-relaxed flex-1">{t(`${p}.steps.${s.key}.p`)}</p>
...
export function Ecosystem() {
  const t = useT();
  const p = 'home.marketing.ecosystem';
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div className="flex flex-col gap-2">
            <EcoItem name={t(`${p}.items.inknoise.name`)} desc={t(`${p}.items.inknoise.desc`)} status="live" />
            <EcoItem name={t(`${p}.items.outline.name`)} desc={t(`${p}.items.outline.desc`)} status="live" link="https://outline.bezier.one" />
            <EcoItem name={t(`${p}.items.app3.name`)} desc={t(`${p}.items.app3.desc`)} status="q4" />
            <EcoItem name={t(`${p}.items.app4.name`)} desc={t(`${p}.items.app4.desc`)} status="2027" />
          </div>
        </div>
      </div>
    </section>
...
function EcoItem({ name, desc, status, link }: { name: string; desc: string; status: 'live' | 'soon' | 'q4' | '2027'; link?: string }) {
  const t = useT();
  const statusLabel = t(`home.marketing.ecosystem.status.${status}`);
...
function FounderCounter() {
  ...
  return (
    <div className="flex items-center gap-3 font-mono-ui text-[11px] tracking-[0.18em] uppercase text-bz-system">
      <span>
        {t('home.marketing.pricing.counter.claimed', {
          count: claimed === null ? '…' : displayed,
          total,
        })}
      </span>
      ...
      <span>{t('home.marketing.pricing.counter.remaining', { count: remaining })}</span>
    </div>
  );
}
