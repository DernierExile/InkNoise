...
import { Manifesto, Pricing, Showcase, Workflow } from './components/marketing/MarketingSections';
      ...
      <main>
        {mode === 'batch' ? (
          ...
          <>
              ...
              <div className="max-w-[1400px] mx-auto px-4 pt-3">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex border border-bz-grid rounded-md overflow-hidden">
                    <button onClick={() => setGalleryMode(true)} className={`px-3 py-1.5 text-[10px] font-mono-ui tracking-wider uppercase ${galleryMode ? 'bg-bz-deep text-bz-paper' : 'text-bz-system'}`}>Galerie</button>
                    <button onClick={() => setGalleryMode(false)} className={`px-3 py-1.5 text-[10px] font-mono-ui tracking-wider uppercase ${!galleryMode ? 'bg-bz-deep text-bz-paper' : 'text-bz-system'}`}>Complet</button>
                  </div>
                </div>
              </div>
            )}

            {(!originalImage || galleryMode) ? (
              <GalleryStudio
                originalImage={originalImage}
                isPro={isPro}
                onImageLoad={handleImageLoad}
                onExit={goHome}
                toolbar={<ModeSwitch mode={mode} isPro={isPro} onChange={handleModeToggle} />}
              />
            ) : (
            ...
            <Workflow />
            <Showcase />
            <Pricing onSignInNeeded={() => setShowAuthModal(true)} />
            <Manifesto />
            <SiteFooter />
          </>
        )}
